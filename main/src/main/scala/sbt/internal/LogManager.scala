...
object LogManager {
  ...
  def defaults(extra: ScopedKey[?] => Seq[XAppender], console: ConsoleOut): LogManager =
    defaults((sk: ScopedKey[?]) => extra(sk).map(new ConsoleAppenderFromLog4J("extra", _)), console)
