...
object TaskSerial extends Properties("task serial") {
  ...
  def checkArbitrary(
      size: Int,
      restrictions: ConcurrentRestrictions[Task[?]],
  ...
  def tagged(f: TagMap => Boolean) = tagged0[Task[?]](_.tags, f)
  def evalRestricted[T](t: Task[T])(restrictions: ConcurrentRestrictions[Task[?]]): T =
