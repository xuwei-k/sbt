...
trait ExecuteProgress[F[_]] {
   ...
   */
  def afterRegistered(task: F[?], allDeps: Iterable[F[?]], pendingDeps: Iterable[F[?]]): Unit
...
object ExecuteProgress {
  def empty[F[_]]: ExecuteProgress[F] = new ExecuteProgress[F] {
    ...
    override def afterRegistered(
        task: F[?],
        allDeps: Iterable[F[?]],
        pendingDeps: Iterable[F[?]]
    ): Unit =
      ()
    override def afterReady(task: F[?]): Unit = ()
    override def beforeWork(task: F[?]): Unit = ()
